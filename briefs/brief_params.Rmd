...
header-includes:
  ...
  - \fancyhf{}
  - \fancyhead[L]{\ifnum\value{page} < 2 {\colorbox{black}{\parbox[b][2.5cm][c]{18.38cm}{\textcolor{white}{\textsf{Eastern Forest Working Group\\ Inventory and Monitoring Program\\ \hspace{1cm}\\Forest Health Monitoring\\ \hspace{1cm}}}}}}\else {\textsf{Eastern Forest Working Group}}\small \fi}
  - \chead[C]{\ifnum\value{page} < 2 {\textcolor{white}{\hspace{7.7cm}\textsf{National Park Service\\ \hspace{9.2cm}U.S. Department of the Interior\\ \vspace{1.1cm}}}} \else \fi}
...
params:
  park: park
---
<!-- - \pretitle{\begin{center}\LARGE\bfseries\vspace{0cm}} -->
...
park_text <- read.csv("../data/parks_mgt_20220923.csv") %>% 
  rename_with(tolower) %>% 
  filter(park == paste(park_info$park))
```
...
\break
\normalsize 
Forests are a key part of the landscape and visitor experience in eastern parks. These ecosystems are facing many stressors, including non-native plants, invasive tree pests, overabundant deer, and an altered fire regime. These stressors diminish forest resilience, which is the ability of an ecosystem to experience disturbance and rebound to similar functions, structure, and composition. A lack of tree regeneration - seedlings and saplings of canopy-forming trees - is an early indicator of reduced resilience and potential for future forest loss.

The NPS Eastern Forest Working Group assessed 10 regeneration metrics in 39 parks containing about 1,500 permanent forest health monitoring plots sampled every four years (2008-2019). Here, we share results for `r paste(park_info$park_full)` (`r paste(park_info$park)`). For more detailed information on the full study, see the source publication in the {\bf Resources} section below.
\end{flushleft}
...
\break
\normalsize
On a four-category scale (Secure, Insecure, Probable Failure, and Imminent Failure), `r paste(park_info$park)` has a forest regeneration status of \textbf{`r paste(park_data$park_reggrp)`}. This means that the park `r ifelse(park_data$park_reggrp == "Imminent Failure", paste("is experiencing severe regeneration failure and has an elevated risk of forest loss. In some cases, this forest loss may already be underway"), ifelse(park_data$park_reggrp == "Probable Failure", paste("is experiencing regeneration failure as indicated by lacking and/or declining sapling numbers, and low numbers of seedlings"), ifelse(park_data$park_reggrp == "Insecure", paste("currently lacks adequate saplings and seedlings, and generally has moderate deer browse impacts"), paste("currently has abundant seedling and sapling numbers, and shows few concerning trends in regeneration metrics"))))` (Figure 2).`r paste(notable)``r ifelse(park_text$findings.2 != "none", paste0(" Additionally, we found ", park_text$findings.2, " in the park."), paste(""))` The trends over time in tree, sapling, and seedling abundance by native canopy, subcanopy, and exotics are shown in Figure 3.
\end{flushleft}
...
`r ifelse(park_text$current.active.deer.management == "yes" & park_text$reccomend.deer.management == "yes", paste0("\\", "item continue protecting forest regeneration from browse impacts through sustained deer culling and fencing."), paste(""))`
`r ifelse(park_text$current.active.deer.management == "no" & park_text$reccomend.deer.management == "yes", paste0("\\", "item implement protection of forest regeneration from browse impacts through sustained deer culling and fencing."), paste(""))`
`r ifelse(park_text$thinning...canopy.gaps == "yes", paste0("\\", "item assess forest stands at risk or already impacted by pests / pathogens and prioritize invasive plant management in stands where canopy gaps have increased / will increase light to the forest floor."), paste(""))`
...
`r ifelse(park_text$thinning...canopy.gaps == "yes", paste0("\\", "item promote native tree regeneration by increasing light to the forest floor through thinning less desirable subcanopy trees in areas with minimal impacts from deer and invasive plants."), paste(""))`
\end{itemize}
...
\tiny {}
\end{flushleft}
{\bf Source Publication:} Miller K., Perles S., Schmit J.P., Matthews E., Weed A., Comiskey J., Marshall M., Nelson P., Fisichelli N. 202X. Forests in eastern national parks face widespread regeneration debt. Ecological Applications.
\begin{flushleft}
\tiny {}
\end{flushleft}
